...
static monoString *FormatCount(int enemy, int bot) {
    char buf[128] = {0};
    sprintf(buf, "[Enemies:%d Bots:%d]", enemy, bot);
    return U3DStr(buf);
}
...
static void spofNick(void *players) {
    ...
    _spof_nick(players, U3DStr("BAPAN FF MODS"));
}

static monoString *U3DStrFormat(float distance) {
    char buffer[128] = {0};
    sprintf(buffer, " [Distance: %.fm]", distance);
...
static Vector2 WorldToScreenPoint(void *WorldCam, Vector3 WorldPos) {
    Vector2 (*_WorldToScreenScene)(void* Camera, Vector3 position) = (Vector2 (*)(void*, Vector3)) getRealOffset(Global.WorldToScreenPoint);
